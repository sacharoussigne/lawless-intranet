...
export function jsonResponse(request: Request, body: unknown, status = 200): NextResponse {
  return withCors(request, NextResponse.json(body, { status }));
}

export function errorResponse(request: Request, error: string, status: number): NextResponse {
