...
import { addDescriptionSuggestion, getDescriptionSuggestions } from '@/lib/domain';
...
export async function GET(request: Request) {
  ...
  const data = await getDescriptionSuggestions(scope.data.scopeType, scope.data.scopeId);
...
export async function POST(request: Request) {
  ...
  const result = await addDescriptionSuggestion(
