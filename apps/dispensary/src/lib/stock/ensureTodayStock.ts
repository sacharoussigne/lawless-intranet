...
async function fetchLatestPreviousForPairs(
  ...
  const relevantGroups = groups.filter((g) => wanted.has(pairKey(g.itemId, g.chestId)) && g._max.timestamp);
...
/**
 * On an inventory action: copy previous stock into today for every missing
 * (item, active chest) pair. Chests already counted today keep their rows.
...
export async function ensureTodayStockForAllActiveChests(
  ...
  await ensureTodayStockForPairs(client, dispensaryId, missingPairs, { today, tomorrow });
}

/**
 * Ensures today's StockHistory rows exist for the given pairs by copying the
 * latest previous-day quantity when missing. Does not create movements.
 */
