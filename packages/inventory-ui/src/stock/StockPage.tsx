...
import { groupItemsByCategory } from '../lib/sortItemsByCategory';
import { resolveLastStockDayLabel } from '../lib/stockPreviousLabel';
...
export default function StockPage({
  ...
  const { permissions, scopeKey } = useInventoryUi();
  ...
  const [editedQuantitiesByItemId, setEditedQuantitiesByItemId] = useState<
  ...
  if (stockItemsInitialData) {
    const key = stockKeys.items(scopeKey, selectedChestId);
  ...
  const { data: stockChecksSummary = initialStockChecksSummary } =
    useStockChecksSummary(initialStockChecksSummary);
  const { data: lastStockDaysByChest = initialLastStockDaysByChest } =
    useLastStockDaysByChest(initialLastStockDaysByChest);
  const { data: visibility = EMPTY_CHEST_STOCK_VISIBILITY } =
    useChestStockVisibility(selectedChestId);
  ...
  const { visibleItems, hiddenItems } = useMemo(
    () =>
      selectedChestId
        ? partitionItemsByVisibility(items, activeVisibility)
  ...
  const isCategoryCheckEnabled = useCallback(
    (categoryId: string): boolean => {
      ...
      return stockChecksSummary.enabledChestIds.some((chestId) =>
        isCategoryEnabledForChest(chestId),
      );
    },
